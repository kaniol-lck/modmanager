...
std::optional<GameVersion> GameVersion::deduceFromString(const QString &string)
{
    ...
    if(re.indexIn(string) != -1){
        //2nd cap
        auto str = re.cap(1);
        if(cachedVersionList.contains(str))
...
void GameVersion::initVersionList()
{
    ...
    if(error.error == QJsonParseError::NoError){
        ...
        for(const auto &entry : qAsConst(list))
            curseforgeVersionList << value(entry, "versionString").toString();
    }
}

QStringList GameVersion::curseforgeVersionList;

QStringList GameVersion::cachedVersionList{
    "1.17.1",
    "1.17",
    "1.16.5",
    "1.16.4",
